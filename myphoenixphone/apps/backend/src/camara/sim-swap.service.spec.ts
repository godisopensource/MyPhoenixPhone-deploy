...
describe('SimSwapService', () => {
  ...
  describe('retrieveSimSwapDate', () => {
    it('returns latestSimChange when phone number contains swapped', async () => {
      ...
      expect(new Date(res.latestSimChange!).toString()).not.toBe(
        'Invalid Date',
      );
    ...
    it('throws error when phone number is empty', async () => {
      await expect(service.retrieveSimSwapDate('')).rejects.toThrow(
  ...
  describe('checkSimSwap', () => {
    ...
    it('throws error when maxAge is out of range', async () => {
      await expect(service.checkSimSwap('+33600123456', 0)).rejects.toThrow(
        'maxAge must be between 1 and 2400 hours',
      );
      await expect(service.checkSimSwap('+33600123456', 2401)).rejects.toThrow(
        'maxAge must be between 1 and 2400 hours',
      );
    });
  });
