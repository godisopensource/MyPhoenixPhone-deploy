...
export class ReachabilityService {
   ...
   */
  async getReachabilityStatus(
    phoneNumber: string,
  ): Promise<ReachabilityStatusResponse> {
   ...
   */
  private async getReachabilityStatusFromApi(
    phoneNumber: string,
  ): Promise<ReachabilityStatusResponse> {
    try {
      ...
      this.logger.debug(
        `Reachability API response: ${JSON.stringify(response.data)}`,
      );
   ...
   */
  private async getReachabilityStatusStub(
    phoneNumber: string,
  ): Promise<ReachabilityStatusResponse> {
