...
export class OAuth2ClientService {
  ...
  private async requestAccessToken(): Promise<OAuth2TokenResponse> {
    const tokenUrl = process.env.CAMARA_TOKEN_URL
    const clientId = process.env.CAMARA_CLIENT_ID
    const clientSecret = process.env.CAMARA_CLIENT_SECRET

    if (!tokenUrl || !clientId || !clientSecret) {
      throw new Error('Missing OAuth2 configuration: CAMARA_TOKEN_URL, CAMARA_CLIENT_ID, or CAMARA_CLIENT_SECRET')
    }
    ...
    try {
      ...
      this.logger.debug(`Access token obtained, expires in ${response.data.expires_in}s`)
      return response.data
    } catch (error) {
      this.logger.error(`Failed to obtain access token: ${error.message}`)
      if (axios.isAxiosError(error) && error.response) {
        this.logger.error(`Token endpoint response: ${JSON.stringify(error.response.data)}`)
      }
