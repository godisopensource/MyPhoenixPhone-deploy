...
export class SimSwapService {
  ...
  constructor(private readonly oauth2Client: OAuth2ClientService) {
    ...
    if (this.useRealApi) {
      ...
      this.logger.warn('SimSwapService using stub mode (missing CAMARA credentials)')
    }
  }
   ...
   */
  async retrieveSimSwapDate(phoneNumber: string): Promise<SimSwapRetrieveDateResult> {
  ...
  private async retrieveSimSwapDateFromApi(phoneNumber: string): Promise<SimSwapRetrieveDateResult> {
    try {
      ...
      this.logger.debug(`SIM Swap API response: ${JSON.stringify(response.data)}`)
   ...
   */
  private async retrieveSimSwapDateStub(phoneNumber: string): Promise<SimSwapRetrieveDateResult> {
    // Deterministic stub: if contains 'swapped' return a recent date
    if (phoneNumber.includes('swapped')) {
      return {
        latestSimChange: new Date(Date.now() - 1000 * 60 * 60 * 24).toISOString(),
        monitoredPeriod: 120
      }
    }
   ...
   */
  async checkSimSwap(phoneNumber: string, maxAge: number = 240): Promise<SimSwapCheckResult> {
    ...
    if (maxAge < 1 || maxAge > 2400) {
      throw new HttpException('maxAge must be between 1 and 2400 hours', HttpStatus.BAD_REQUEST)
    }
  ...
  private async checkSimSwapFromApi(phoneNumber: string, maxAge: number): Promise<SimSwapCheckResult> {
    try {
      ...
      this.logger.debug(`SIM Swap check API response: ${JSON.stringify(response.data)}`)
      return response.data
    } catch (error) {
   ...
   */
  private async checkSimSwapStub(phoneNumber: string): Promise<SimSwapCheckResult> {
    if (phoneNumber.includes('swapped')) {
      return { swapped: true }
    }
