  ...
  async verifyNumber(
    ...
    try {
      ...
      if (result.codeSent) {
        ...
        console.log(
          `[VerificationController] Demo mode check: isDemoMode=${isDemoMode}, CAMARA_ENV=${process.env.CAMARA_ENV}, USE_MCP_PROXY=${process.env.USE_MCP_PROXY}, MCP_PROXY_URL=${process.env.MCP_PROXY_URL}, result.code=${result.code}`,
        );
        ...
        if (isDemoMode && result.code) {
          ...
          console.log(
            `[VerificationController] ⚠️ Demo mode active but no code returned from service`,
          );
        }
